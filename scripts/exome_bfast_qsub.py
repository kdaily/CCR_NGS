...
def check_final_output(samples, config):
    """Take a subset of the tasks after checking that the final BAM file was written.
    
    This will save from having to re-write the config files.
    
    """

    bam_output_dir = config['bamindex_params']['output_dir']

    keep_samples = []

    for sample in samples:

        file_path = os.path.join(bam_output_dir, "%(flowcell_id)s_%(sample_name)s.bam" % sample)
        # logger.debug("Checking %s" % file_path)

        if not os.path.exists(file_path):
            # logger.debug("File %s not found, appending." % file_path)
...
# samples = check_final_output(samples, config)
# samples = samples[:1]

# setup inital run params
initial_task_params = bfast_helpers.make_sickle_param_list(samples=samples, config=config, params=None)
...
initial_task_params = [x for x in initial_task_params if x[2]['sample_name'] in ["MN19", "MT19"]] #, "MN20", "MT20", "MN21", "MN22", "MN23"]]
bfast_match_task_params = [x for x in bfast_match_task_params if x[2]['sample_name'] in ["MN19", "MT19"]] #, "MN20", "MT20", "MN21", "MN22", "MN23"]]
...
# print bfast_match_task_params[0]
# sys.exit(0)

def check_bfast_match(input, output, params=None):
    """Check if posttask file for bfast match was created.
    """
    
    sentinel_file = "%s/bfast_match_completed.flag" % config['bfast_match_params']['output_dir']
    logger.debug(sentinel_file)

    if not os.path.exists(sentinel_file):
        return True, "Missing file %s" % output
    else:
        return False, "bfast match completed."

def check_bfast_localalign(input, output, sample_name=None, index=None, ref_index=None):
    """Check if posttask file for bfast localalign was created.
    """
    
    sentinel_file = "%s/bfast_localalign_completed.flag" % config['bfast_localalign_params']['output_dir']
    logger.debug(sentinel_file)
...
#         mkdir(config['bfast_mergematch_params']['output_dir']),
         mkdir(config['bfast_localalign_params']['output_dir']),
         mkdir(config['bfast_postprocess_params']['output_dir']),
         mkdir(config['picard_sortsam_params']['output_dir']),
         mkdir(config['mergebam_params']['output_dir']),
#         mkdir(config['gatk_realigner_target_creator_params']['output_dir']),
#         mkdir(config['gatk_indel_realigner_params']['output_dir']),
#         mkdir(config['mutect_params']['output_dir']),
)
...
def run_sickle(input, output, params=None):
    ...
    cmd = "%(exec)s pe --quiet -t sanger -l %(length)s -q %(quality)s -c %(input)s -m /dev/stdout -s %(output)s.singles | gzip > %(output)s" % params
...
@follows(run_sickle, mkdir(config['bfast_match_params']['output_dir']))
@files(bfast_match_task_params)
# @transform(run_sickle, regex(r".*/(.*)_(.*)_(.*)_(.*).fastq"), r"%s/\1_\2_\3_\4.baf" % config['bfast_match_params']['output_dir'], r"\2", r"\4")
# @posttask(touch_file("%s/bfast_match_completed.flag" % config['bfast_match_params']['output_dir']))
# def run_bfast_match(input, output, sample_name, ref_index, params=None):
def run_bfast_match(input, output, params=None):
    ...
    bfast_params['ref_index'] = params['ref_index']

    cmdline = "--bfast_temp_dir=%(temp_dir)s --bfast_threads=%(threads)s --bfast_reference_fasta=%(reference_fasta)s --bfast_output_file=%(output)s match --bfast_gzipped --bfast_reads_file=%(input)s --bfast_space=0 --bfast_main_indexes=%(ref_index)s" % bfast_params
...
def run_merge_bfastmatch(input, output, sample_name=None, index=None):
    ...
    # Update input and output from global config object
    mergebam_params = config['bfast_mergematch_params']
    ...
    cmd = "%(exec)s %(input)s > %(output)s" % mergebam_params

    job_id = utils.safe_qsub_run(cmd, jobname="mbmf%s%s" % (params['sample_name'], params['index']),
...
def run_bfast_localalign(input, output, sample_name=None, index=None):
    ...
    job_id = utils.safe_qsub_run(bfast_command, jobname="bfla%s%s" % (params['sample_name'], params['index']),
                                 nodes=bfast_params['qsub_nodes'], params="-l walltime=168:00:00",
                                 stdout=stdout, stderr=stderr)
...
@collate(run_split_bam, regex(r".*/(.*)_(M[NT])(.*)\.bam"),  r"%s/\1_\3.callstats" % config['mutect_params']['output_dir'],  r"%s/\1_\3_coverage.wig" % config['mutect_params']['output_dir'])
def run_mutect(input, output, coverage_file, params=None):
    """Run mutect.
    ...
    cmd = "%(java_exec)s -Xmx%(maxjheap)s -Djava.io.tmpdir=%(tmp_dir)s -jar %(jar_file)s --analysis_type MuTect --dbsnp %(dbsnp_file)s  --input_file:normal %(input_normal)s --input_file:tumor %(input_tumor)s --reference_sequence %(reference_fasta)s --out %(call_stats_file)s --coverage_file %(coverage_file)s" % params
...
job_list_runfast = [run_mk_output_dir, run_sickle]
...
job_list_GATK = [run_realign_indel_creator, run_indel_realigner, run_base_score_recalibrator, run_write_recalibrated_bam, run_split_bam, run_mutect]
## job_list_bfast = [run_mark_duplicates, run_indexbam, run_realign_indel_creator]
job_list_rest = []

job_list = job_list_runfast + job_list_bfast # + job_list_postprocess + job_list_GATK + job_list_rest
